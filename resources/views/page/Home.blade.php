...
<body>
    ...
    <!-- MODAL INBOX -->
    <div id="inbox-modal" tabindex="-1" aria-hidden="true"
        class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div
            class="relative w-full max-w-2xl max-h-full md:h-auto right-0 mt-11 mr-auto left-auto md:left-1/2 lg:left-1/2">
            <div class="relative bg-gray-200 rounded-lg shadow dark:bg-gray-700">
                <div
                    class="flex justify-between items-center p-5 rounded-t border-b border-gray-200 dark:border-gray-600">
                ...
                <div class="p-6 space-y-6">
                    @if ($notifications->isEmpty())
                        <p class="text-gray-500 dark:text-gray-400">You don't have any notifications.</p>
                    @else
                        @foreach ($notifications as $notification)
                            <p class="text-gray-500 dark:text-gray-400">Selamat! Anda telah memenagkan sebua lelang
                                untuk {{ $notification->barang->nama_barang }}. Tolong contact petugas sekarang!</p>
                        @endforeach
                    @endif
                </div>
            </div>
    ...
    <section
        ...
        @if ($lelangs->isEmpty())
            <h2 class="text-center text-2xl font-bold text-gray-900 dark:text-white">
                tidak ada lelang untuk saat ini, kembali lain kali :)
            </h2>
        @else
